...
class HHSimulatorGUI:
    def __init__(self, master):
        ...
        self.neuron_state_label = tk.Label(self.status_frame, text='', font=('Arial', 24))
        self.legend_txt = []
    ...
    def create_main_frames(self):
        ...
        self.input_frame.grid(row=1, column=0, sticky='SW', pady=5, padx=5)
        ...
        self.plot_frame.grid(row=1, column=1, sticky='NW', pady=5, padx=5)


    def create_widgets(self):
        """Creates the widgets, titles and buttons of the window"""
        # Labels
        self.main_title = tk.Label(self.master, text="Parameters:", font=('Arial', 24))
        self.main_title.grid(row=0, column=0, pady=10)

        self.amp_title = tk.Label(self.input_frame, text='Current step amplitude [nano Amper]', font=('Arial', 20))
        self.amp_title.grid(row=0, column=0, pady=10, sticky='W')

        self.time_title = tk.Label(self.input_frame, text='Experiment duration [milli second]', font=('Arial', 20))
        self.time_title.grid(row=2, column=0, pady=10, sticky='W')

        self.neuron_status_headline = tk.Label(self.status_frame, text='Neuron status:', font=('Arial', 24))
        self.neuron_status_headline.grid(row=0, column=0, pady=10, padx=10)

        # Entry spinboxes 
        self.amp_spinbox = tk.Spinbox(self.input_frame, from_=1, to=100, increment=5, width=5, font=('Arial', 20))
        self.amp_spinbox.grid(row=1, column=0, pady=10)
        self.amp_spinbox.delete(0, tk.END)
        self.amp_spinbox.insert(0, 15)

        self.time_spinbox = tk.Spinbox(self.input_frame, from_=1, to=100, increment=5, width=5, font=('Arial', 20))
        self.time_spinbox.grid(row=3, column=0, pady=10)
        ...
        self.button_plot = tk.Button(self.buttons_frame, text="Plot data", command=lambda:
                                      self.plot_data(float(self.amp_spinbox.get()),
                                                      int(self.time_spinbox.get())), font=('Arial', 20))
        self.button_plot.grid(row=0, column=0, sticky='SW', pady=10, padx=5)

        self.button_delete = tk.Button(self.buttons_frame, text="Erase data", command=self.erase_data, font=('Arial', 20))
        ...
        self.stack_checkbox = tk.Checkbutton(self.buttons_frame, text='Stack plots',
                                              variable=self.stack_plots, onvalue=1, offvalue=0, font=('Arial', 20))
        self.stack_checkbox.grid(row=1, column=0, sticky='SW', pady=10, padx=5)

        self.N_active_checkbox = tk.Checkbutton(self.buttons_frame, text='Deactivate N',
                                                 variable=self.N_active, onvalue=False, offvalue=True, font=('Arial', 20))
        self.N_active_checkbox.grid(row=2, column=0, sticky='SW', pady=5, padx=5)
        
        self.M_active_checkbox = tk.Checkbutton(self.buttons_frame, text='Deactivate M',
                                                 variable=self.M_active, onvalue=False, offvalue=True, font=('Arial', 20))
        self.M_active_checkbox.grid(row=3, column=0, sticky='SW', pady=5, padx=5)
        
        self.H_active_checkbox = tk.Checkbutton(self.buttons_frame, text='Deactivate H',
                                                 variable=self.H_active, onvalue=False, offvalue=True, font=('Arial', 20))
        self.H_active_checkbox.grid(row=4, column=0, sticky='SW', pady=5, padx=5)
    ...
    def plot_data(self, i_inj, time_tot):
        ...
        self.action_potential = FigureCanvasTkAgg(self.action_fig, master=self.plot_frame)
        self.action_potential.draw()
        self.action_potential.get_tk_widget().grid(row=0, column=0, pady=2)

        self.dynamics_plot = FigureCanvasTkAgg(self.dynamics_fig, master=self.plot_frame)
        self.dynamics_plot.draw()
        self.dynamics_plot.get_tk_widget().grid(row=1, column=0, pady=2)

        self.step_current = FigureCanvasTkAgg(self.step_current_fig, master=self.plot_frame)
        self.step_current.draw()
        self.step_current.get_tk_widget().grid(row=0, column=1, pady=2, padx=5)
